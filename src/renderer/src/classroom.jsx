...
const ClassroomPage = () => {
    ...
    const [currentUser, setCurrentUser] = useState({ id: '', role: 'user', token: '', username: 'Guest' });
    const [classroomToken, setClassroomToken] = useState('Loading...');
    const [members, setMembers] = useState([]);
    const [showAdminActions, setShowAdminActions] = useState(false);
    const [joinToken, setJoinToken] = useState('');
    const [addMemberId, setAddMemberId] = useState('');
    const [editorContent, setEditorContent] = useState('// Start coding together in real-time...\n// Changes sync across all members!');
    ...
    const renderMembers = () => {
        return members.map((member, index) => (
            <li key={index} className="member-item" style={{
        ...
        <div id="main">
                ...
                <div className="nav-bar">
                    ...
                    <div className="menu">
                        <div className="logo-toggle">
                            <span className="logo"><a href="/">Human Error</a></span>
                    ...
                    <div className="darkLight-searchBox">
                        ...
                        <a href="/account/login" className="login">Login</a>
                    </div>
                </div>
            </nav>
            ...
            <div className="content" style={{
                ...
                <div className="glass-container" style={{
                    ...
                    {/* Header */}
                    <div className="classroom-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <span className="classroom-title" style={{ fontSize: '2rem', fontWeight: '700', color: '#222', letterSpacing: '1px' }}>
                            Classroom
                        </span>
                    ...
                    {/* Token Area */}
                    <div className="token-area" style={{
                        ...
                        <i
                            ...
                            title="Copy token"
                            style={{ cursor: 'pointer', color: '#007bff', fontSize: '1.3rem', marginLeft: '1rem' }}
                            onClick={copyToken}
                        />
                    </div>

                    {/* Join Area */}
                    <div className="join-area" style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                        <input
                            type="text"
                            className="join-input"
                            placeholder="Paste classroom code/token to join..."
                            style={{ flex: 1, padding: '0.7rem 1rem', borderRadius: '8px', border: '1px solid #ccc', fontSize: '1rem', background: 'rgba(255,255,255,0.7)' }}
                            value={joinToken}
                            onChange={(e) => setJoinToken(e.target.value)}
                        />
                        <button
                            className="join-btn"
                            style={{ padding: '0.7rem 1.2rem', borderRadius: '8px', border: 'none', background: '#007bff', color: '#fff', fontWeight: '600', cursor: 'pointer' }}
                    ...
                    {/* Members Area */}
                    <div className="members-area" style={{ background: 'rgba(255,255,255,0.4)', borderRadius: '12px', padding: '1rem', marginTop: '1rem' }}>
                        <div className="members-title" style={{ fontSize: '1.1rem', fontWeight: '600', marginBottom: '0.5rem', color: '#333' }}>
                            Members
                        </div>
                        <ul className="member-list" style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                            {members.length === 0 ? (
                                <li>Loading...</li>
                            ) : (
                                renderMembers()
                        ...
                        {showAdminActions && (
                            <div id="admin-actions" style={{ marginTop: '1rem' }}>
                                <input
                                    ...
                                    style={{ padding: '0.3rem 0.7rem', borderRadius: '6px', border: '1px solid #ccc' }}
                                    value={addMemberId}
                                    onChange={(e) => setAddMemberId(e.target.value)}
                                />
                                <button
                                    onClick={addMember}
                                    style={{ marginLeft: '0.5rem', padding: '0.3rem 0.8rem', borderRadius: '6px', border: 'none', background: '#007bff', color: '#fff' }}
                                >
                                    Add Member
                                </button>
                            </div>
                    ...
                    {/* Editor Area */}
                    <div className="editor-area" style={{ marginTop: '2rem', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
                        <textarea
                            id="editor"
